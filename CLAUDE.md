...
**WhatsApp sending is manual, not automated.** The backend returns a precomputed `whatsappLink` (a `https://wa.me/...` URL) on family/amigo records whenever a `telefono` is present; `renderWhatsappBtn()` (script.js:19) renders that as a link that opens WhatsApp with the message pre-filled, and reception clicks "send" themselves. There's no bulk-send endpoint in active use — `POST /guests/enviar-invitaciones` exists on the backend but is intentionally not called from this front end (depends on an unapproved Twilio/WhatsApp Business account).
