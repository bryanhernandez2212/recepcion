...
- Familias are the CRUD-driven family/table records (`/guests`), always with a `count` of invited passes and optionally a `table` (any positive integer — the backend no longer caps this at 24, see the `/mesas` note below for a related gap).
...
**Mesas (`/mesas`)** returns a fixed range of tables (still ~1–24, even empty ones) with their assigned familias embedded — clicking a table tile does not trigger another fetch, it just renders `mesa.familias` from the already-loaded `mesas` array (see `openMesaDetail`/`renderMesaDetail`). Amigos never have a table and never appear here. Note: since the front's table-number validation no longer caps at 24 (a family can be assigned any positive table number), a family assigned to a table outside `/mesas`'s fixed range currently won't show up in the Mesas tab — that's a backend gap, not a front-end bug.
...
**Validation is duplicated client-side** to match backend constraints before submitting, so failures surface immediately in the form rather than as a generic API error: `validTable()` only requires a positive integer (no upper bound — the old 1–24 cap was removed since the backend now accepts any table number), `validTelefono()` enforces exactly 10 digits. Keep these in sync with the backend's rules described in `correcion.md` if either changes.
